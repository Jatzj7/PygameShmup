...
def OptionsMenu():
    ...
    draw_text(screen, "Press Space to go back",
              18, game_width/2, game_height*3.4/4)
    ...
    Options = True
    while Options:
        button(game_width/2-130, 150, 40, 31,
               player_options_img_2, 'player_options_img_2')
        button(game_width/2-20, 150, 40, 31,
               player_options_img_3, 'player_options_img_3')
        button(game_width/2+90, 150, 40, 31,
               player_options_img_4, 'player_options_img_4')
        ...
        button(game_width/2+40, 400, 80, 80,
               background_options_2, 'background_2')
        button(game_width/2-110, 400, 80, 80,
               background_options_3, 'background_3')
        clock.tick(fps)
...
def show_go_screen():
    ...
    draw_text(screen, "Arrow Keys to move,Space to Fire",
              22, game_width/2, game_height/2)
    draw_text(screen, "Press Space to begin",
              18, game_width/2, game_height*3/4)
    draw_text(screen, "Press d for Options", 18,
              game_width/2, game_height*3.2/4)
    ...
    draw_text(screen, "High Score" +
              str(player.high_Score), 22, game_width/2, 15)
...
class Powerups(pygame.sprite.Sprite):
    def __init__(self, center):
        pygame.sprite.Sprite.__init__(self)
        self.type = random.choice(['shield', 'gun', 'life','Temp_shield'])
...
class Shield(pygame.sprite.Sprite):
    def __init__(self, center):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.transform.scale(sheild_img, (100, 30))
        self.image.set_colorkey(black)
        self.rect = self.image.get_rect()
        self.rect.center = center
        self.rect.bottom = game_height-50
        self.speedx = 0
    ...
    def update(self):
        ...
        if keystate[pygame.K_RIGHT]:
            self.speedx = 5
        self.rect.x += self.speedx
        if self.rect.right > game_width+20:
            self.rect.right = game_width+20
        if self.rect.left < -20:
            self.rect.left = -20
        self.center = player.center
        if pygame.time.get_ticks() - self.time > shield_time:
...
sheild_img = pygame.image.load(os.path.join(img_path, "shield1.png")).convert()
sheild_img.set_colorkey(black)
...
for meteor_img in meteor_list:
    meteor_shower.append(pygame.image.load(
        os.path.join(img_path, meteor_img)).convert())
explosion_animation = {}
...
powerup_img['Temp_shield'] = pygame.image.load(os.path.join(img_path, 'pill_yellow.png')).convert()
...
for explosion_sound in ["explosion_sound2.wav", "explosion_sound1.wav"]:
    explosion_sounds.append(pygame.mixer.Sound(
        os.path.join(snd_path, explosion_sound)))
pygame.mixer.music.load(os.path.join(
    snd_path, "tgfcoder-FrozenJam-SeamlessLoop.ogg"))
...
while running:
    ...
    # Checking the shield mobs
    hit_sheilds = pygame.sprite.groupcollide(shields, mobs, False, True)
    for hit_sheild in hit_sheilds:
        random.choice(explosion_sounds).play()
        newmob()
    for hit in hits:
        random.choice(explosion_sounds).play()
